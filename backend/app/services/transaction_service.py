...
async def _get_series_transactions(
    ...
    Series identity = workspace + account + installment_purchase_date +
    total_installments (the fingerprint the sync dedup also matches). For
    "future" only rows with installment_number >= the anchor's are returned.
    """
    conditions = [
        Transaction.workspace_id == workspace_id,
