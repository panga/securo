...
/** Heuristic: detect descriptions that are long strings without spaces
 * (e.g. bank reference IDs like "MERCADOLIVRE*PRODUTOSINF...").
 * These should be truncated rather than wrapping, since they have no
 * word break opportunities and would break the layout. */
...
interface MobileTransactionRowProps {
  ...
  /** Show payee instead of account info (for per-account views) */
  showPayee?: boolean
}
...
export function MobileTransactionRow({
  ...
  // Virtual rows (non-materialized recurring projections) are informational:
  // never clickable, never selectable, and rendered with a projected badge.
